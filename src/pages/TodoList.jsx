...
const TodoList = () => {
    ...
    return (
       <div className="container m-auto relative overflow-x-auto py-5">
            <table cellPadding={10} className="w-full text-sm rtl:text-right text-gray-500 dark:text-gray-400 text-center rounded-md">
                ...
                <tbody>
                    {
                        list.map((task, index) => {
                            return <tr key={index} className={`  text-xl ${task.status == 'pending' ? 'bg-gray-400' : task.status == 'completed' ? 'bg-green-400' : 'bg-yellow-400'}`}>
                                ...
                                <td className=" grid justify-center md:grid-cols-2 grid-cols-1 gap-5 ">
                                    <button className=' bg-red-400 rounded-md capitalize text-white hover:bg-red-600 w-[120px] h-[50px]' onClick={() => deleteTask(index)}>delete</button>
